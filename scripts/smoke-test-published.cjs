/**
 * Smoke test for the published config-disassembler npm package.
 * Uses fixtures/deeply-nested-unique-id-element/Get_Info.flow-meta.xml.
 * Run from a directory where `npm install config-disassembler` has been run.
...
async function main() {
  ...
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "config-disassembler-smoke-"),
  );
